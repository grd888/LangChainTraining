...
with st.sidebar:
    ...
    user_id = st.text_input("Enter you name", user_id)
    role = st.radio(
        "How detailed should your answer be?", ["Beginner", "Expert", "Phd"], index=0
    )
...
def invoke_history(chain, session_id, prompt):
    ...
    for response in history.stream(
        {"prompt": prompt}, config={"configurable": {"session_id": session_id}}
    ):
        yield response
...
if prompt:
    ...
    with st.chat_message("assistant"):
        response = st.write_stream(invoke_history(chain, user_id, prompt))
